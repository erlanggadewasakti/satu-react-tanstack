...
- **Basepath-Relative Navigation (`VITE_APP_BASE_NAME`)**: All paths passed to `navigate({ to: ... })` or `<Navigate to="..." />` MUST be relative to the router basepath (e.g. `/home` or `/kurikulum/home`, NEVER `/lens/home`). When running behind a reverse proxy (e.g. `VITE_APP_BASE_NAME=/lens`), TanStack Router handles the basepath prefix automatically. Passing a basepath-prefixed string into `navigate()` causes double prefixing (`/lens/lens/...`) and breaks route matching.
- **Never Put Mutable Router State Objects in `useEffect` Dependencies**: In TanStack Router, `location.state` produces a new object reference on every navigation call. Never pass `location.state` or `location` directly into `useEffect` dependency arrays. Always extract primitive values (e.g. `const fromPath = location?.state?.from`) to prevent infinite navigation and re-render loops (`Maximum update depth exceeded` / browser Out of Memory).
- **Single-Shot Guard Navigation (`hasNavigated` Ref)**: Route guards (`AuthGuard`, `GuestGuard`) MUST use a `useRef(false)` flag to ensure `navigate()` is dispatched strictly once per authentication state change.
