...
export default function NavCollapse({ menu, level, parentId, setSelectedItems, selectedItems, setSelectedLevel, selectedLevel }: Props) {
  ...
  const handleClickCollapse = (event: MouseEvent<HTMLAnchorElement> | MouseEvent<HTMLDivElement, MouseEvent>) => {
  ...
  const handleClick = (
    event: MouseEvent<HTMLButtonElement> | MouseEvent<HTMLAnchorElement> | MouseEvent<HTMLDivElement, MouseEvent> | undefined,
    isRedirect: boolean
  ) => {
    setAnchorEl(null);
